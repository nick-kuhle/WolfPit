...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  // F14: OAuth initiation is a credential surface too — throttle it by IP via
  // the shared DB (fail-closed), so a bot cannot hammer the broker redirect.
  const { throttledByIp } = await import("./rate-limit.server");
  if (await throttledByIp(request)) {
    return completionResponse({ source: "wolfpit-auth-popup", token: null, error: "rate_limited" });
  }
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // redirect_uri for the dynamic preview-host origin.
...
<script type="application/json" id="wolfpit-auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("wolfpit-auth-popup-msg");
  var msg = { source: "wolfpit-auth-popup", token: null };
