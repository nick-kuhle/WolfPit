 ...
 * (`config/app-env.json`), which selects the dev user (see `use-current-user`);
...
const BEARER_KEY = "wolfpit-auth.bearer-token";
...
/**
 * True when the app runs inside a hosted live preview iframe (host matches
 * `VITE_PREVIEW_HOST_SUFFIX`) — where a full-page redirect to the broker can't
 * work — so sign-in uses a popup there and a normal redirect everywhere else.
 * Empty suffix (the default) ⇒ never live-preview mode.
 */
...
type PopupMessage = { source: "wolfpit-auth-popup"; token: string | null; error?: string };

/**
 * Start sign-in with one upstream provider (`providerId` from `AUTH_PROVIDERS`),
 * federating through the auth broker.
 *
 * - **Live preview** (host matches `VITE_PREVIEW_HOST_SUFFIX`): opens a POPUP to
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `wolfpit-signin-${Date.now()}`;
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.source !== "wolfpit-auth-popup") return;
