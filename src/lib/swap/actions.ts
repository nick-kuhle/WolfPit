...
/**
 * Server function: get a DEX-aggregator quote for a spot swap.
 *
 * The 0x API key stays server-side (see quote.server.ts). Indicative pricing
 * omits `taker`; a firm executable quote (with a signed-ready transaction)
 * requires the connected wallet address in `taker`.
 */
