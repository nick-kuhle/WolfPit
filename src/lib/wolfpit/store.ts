...
import type { DeskProduct } from "./desk-gate";
...
async function serverGate(product: DeskProduct): Promise<string | null> {
  try {
    ...
    // `store.ts` is reachable from `__root.tsx`, so a static import puts the
    // server-function RPC shim in the ROOT bundle chunk, where it evaluates
    // before the Start runtime that defines `createSsrRpc` - every route then
    // 500s on import. Deferring the import to call time keeps the shim in its
    // own chunk. Verified by `npm run test:ssr`, which boots the built bundle.
    const { deskOpen } = await import("./desk-gate");
