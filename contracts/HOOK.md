...
| --- | --- |
| Pool depth is the ONLY listing oracle | §3, §4 (`poolDepthBase` / `poolDepthQuote` → vault) |
...
| Fee tier | The pool uses the hook's **dynamic fee**; the static `fee` field in `PoolKey` is a floor/scaffold (see §5). |
...
The vault exposes a **single, minimally-privileged** depth entry point the hook
...
IWolfPitHook public pitHook;               // verified in zero-hook address
...
/// @notice only callable by `pitHook`. Reports realized depth from a pool action.
///         Written AFTER the pool state is final (afterSwap / afterAddLiquidity /
///         afterRemoveLiquidity) — never a pre-state guess.
function setPoolDepth(uint256 baseDepth, uint256 quoteDepth) external nonReentrant {
    if (msg.sender != address(pitHook) || pitHook == IWolfPitHook(address(0))) revert NotHook();
    poolDepthBase = baseDepth;
    poolDepthQuote = quoteDepth;
    lastDepthAt = block.timestamp;
    lastDepthBlock = blockhash(block.number - 1);
    emit PoolDepth(baseDepth, quoteDepth, lastDepthAt);
}
```

```solidity
// new — the interface the hook expects the vault to satisfy
interface IWolfPitDepthSink {
    function setPoolDepth(uint256 baseDepth, uint256 quoteDepth) external;
}

// readonly the hook exposes for the vault (used by the desk / keeper)
interface IWolfPitHook {
    function poolKey() external view returns (PoolKey memory);
    function currentFeeBps() external view returns (uint24);   // last returned fee
...
- `setPoolDepth` is **write-once-per-block-per-callback**, `nonReentrant`, and
  `onlyHook`. Reentrancy risk is nil because the hook never holds a delta and never
  calls back into the vault synchronously — it only writes a few scalar words. But
  the `nonReentrant` guard stays so a future hook change cannot re-enter.
...
```
RV_t  = λ · RV_{t-1} + (1 − λ) · |ln(P_t / P_{t-1})| · √(365.25·24·60)
λ     = 0.94                                   (RiskMetrics EWMA, per engine.ts / math.ts)
...
function beforeSwap(
    ...
    // Dynamic fee: the pool uses a dynamic-fee fee tier; the returned uint24 is the
    // fee for THIS swap. Enforce the spec formula and clamp.
    uint24 fee = _dynamicFee(rv());
    return (BaseHook.beforeSwap.selector, BeforeSwapDeltaLibrary.ZERO_DELTA, fee);
}

function _dynamicFee(uint256 _rv) internal pure returns (uint24) {
    // _rv is annualized, 18-dec. 0.40 and 5/30 bps are the spec constants.
    uint256 fee = 5 + (80 * (_rv > 0.40e18 ? (_rv - 0.40e18) : 0) / 1e18);
    return uint24(fee < 5 ? 5 : (fee > 30 ? 30 : fee));  // clamp [5,30]
}
```

- `beforeSwap` is `view` (no state write) — it only reads the accumulator.
- Clamp to `[5,30]` bps. `fee` is `uint24` in bps (v4 fee unit). `MaxFee` invariant holds.
...
- The existing `spotFeeBps()` in `engine.ts` must produce the **same number** as
...
| I7 | The hook's `beforeSwap` never allows a fee below 5 or above 30 bps, and never a fee that the sim's `spotFeeBps` would disagree with for the same RV. | hook (§5) |
...
- **The hook is only called by the PoolManager** (`poolManagerOnly`). It makes **zero**
  external calls of its own — no token transfers, no vault calls that it triggers
  synchronously. Its external call is only `setPoolDepth` **if** the vault is set; but
  to keep the hook purely observer-based, prefer the vault **pulling** depth lazily
  (keeper reads `poolDepth*` after a swap) over the hook **pushing** synchronously.
  **Decision:** hook does **not** call the vault. The vault/keeper **reads** the hook's
  stored depth OR the vault stores depth only via the keeper's `reconcileDepth`
  (an `onlyOperator` keeper call that reads the hook and writes the vault). This moves
  all reentrancy risk into the vault's existing nonReentrant surface and keeps the hook
  `view`-only except the vol accumulator.
- The vol-accumulator `afterSwap` write is the **only** state the hook mutates, and it
  is `nonReentrant`-safe (no nested calls). Keep it O(1).
- Do **not** put `setPoolDepth` write logic inside the actual `afterSwap/afterAdd` on
  the hook if the vault is on a different code path — it creates a coupling where a
  vault bug can brick the pool. The hook must never revert the underlying
  swap/liquidity move. **Every callback path returns a valid selector even on an
  internal error**; if depth/vol computation would revert, it is skipped, not thrown.
...
> vault revert can never bubble back into the pool action — i.e. `setPoolDepth` called
> via `try/catch` from the hook, swallowing the vault error and returning the normal
...
- `beforeSwap` / `afterSwap` / `afterAddLiquidity` / `afterRemoveLiquidity` are each
  O(1) (a handful of SSTOREs + a bounded EWMA update). Targets: < 40k gas per callback
  path on Base.
...
| T2 | `beforeSwap` returns a fee in `[5,30]`; with `RV=0.40` → 5, `RV=0.60` → 21, `RV≥0.875` → 30. Matches `spotFeeBps` for the same RV. |
...
| T6 | **Stale depth = 0:** `setPoolDepth` skipped for > N blocks → keeper treats `usable = 0` and refuses to size. |
| T7 | **Same-block collapse:** two swaps in one block produce one vol update; a flash-loop cannot spike RV past a sane bound. |
| T8 | **Fee bounds invariant:** across a fuzzed RV sweep, `_dynamicFee(rv)` never exits `[5,30]`. |
